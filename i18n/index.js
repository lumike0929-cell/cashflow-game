...
export function t(key, params = {}, options = {}) {
  ...
  if (locale === "en" && containsCjk(value)) {
    if (!missingKeys.has(`${locale}:${key}`) && isDevMode()) {
      missingKeys.add(`${locale}:${key}`);
      console.warn(`[i18n] English translation falls back to CJK text: ${key}`);
    }
    return typeof options.fallback === "string" ? interpolate(options.fallback, params) : "Translation unavailable";
  }
...
export function translateText(value, params = {}) {
  ...
  if (exact) {
    if (currentLocale === "en" && containsCjk(exact)) {
      const converted = translateCommonPatterns(original, pack, fallbackPack);
      if (!containsCjk(converted) && converted !== original) return interpolate(converted, params);
      warnMissingEnglishText(original);
      return typeof params.fallback === "string" ? params.fallback : "Translation unavailable";
    }
    return interpolate(exact, params);
  }
  if (currentLocale === "en" && containsCjk(original)) {
    const converted = translateCommonPatterns(original, pack, fallbackPack);
    if (!containsCjk(converted) && converted !== original) return interpolate(converted, params);
    warnMissingEnglishText(original);
    return typeof params.fallback === "string" ? params.fallback : "Translation unavailable";
...
function warnMissingEnglishText(original) {
  const key = `en:text:${original.slice(0, 80)}`;
  if (typeof window !== "undefined") {
    const current = Array.isArray(window.__cashflowMissingEnglishText) ? window.__cashflowMissingEnglishText : [];
    window.__cashflowMissingEnglishText = [...new Set([...current, original])].slice(-80);
  }
  if (!missingKeys.has(key) && isDevMode()) {
    missingKeys.add(key);
    console.warn(`[i18n] Missing English text mapping: ${original}`);
  }
}
...
function translateCommonPatterns(original, pack, fallbackPack) {
  let next = original;
  if (currentLocale === "en") {
    next = next.replace(/房贷购买还差\s*(¥[\d,]+)/g, "Need $1 more to buy with a mortgage");
    next = next.replace(/房貸購買還差\s*(¥[\d,]+)/g, "Need $1 more to buy with a mortgage");
    next = next.replace(/现金购买还差\s*(¥[\d,]+)/g, "Need $1 more to buy with cash");
    next = next.replace(/現金購買還差\s*(¥[\d,]+)/g, "Need $1 more to buy with cash");
  }
...
const textMaps = {
  ...
  en: {
    ...
    "最近徽章": "Recent Badge",
    "现金流": "Cash Flow",
    "现金流资产挑战": "Cashflow Asset Challenge",
    "财务自由模式": "Financial Freedom Mode",
    "被动收入 {amount} 已覆盖必要支出 {expenses}。": "Passive income {amount} now covers essential expenses {expenses}.",
    "地图": "Map",
    "收起": "Collapse",
    "前进": "Move",
    "前进 1 格": "Move 1 Space",
    ...
    "骰": "Roll",
    "保留现金": "Keep Cash",
    "完成第一次掷骰": "Finish Your First Roll",
    "打开财务面板": "Open the Finance Panel",
    "查看月现金流": "Check Monthly Cash Flow",
    "完成第一次事件": "Finish Your First Event",
    "查看一次为什么": "Read One Why",
    "检查紧急预备金": "Check Your Emergency Fund",
    "学习第一项资产": "Learn Your First Asset",
    "查看财务自由进度": "Check Financial Freedom Progress",
    "完成新手教学": "Finish the Beginner Guide",
    "家长／老师说明": "Parent / Teacher Guide",
    "家長／老師說明": "Parent / Teacher Guide",
    "这款游戏在练习什么？": "What does this game practice?",
    "现金流冒险城用简化规则帮助孩子讨论收入、支出、资产、负债、保险、税务和风险。它不是现实投资、保险、法律或税务建议，也不会收集儿童个人资料。": "Cashflow Adventure City uses simplified rules to help children discuss income, expenses, assets, liabilities, insurance, taxes, and risk. It is not real investment, insurance, legal, or tax advice, and it does not collect children's personal information.",
    "适合讨论": "Good Discussion Questions",
    "为什么高收入不一定代表高现金流？": "Why does high income not always mean high cash flow?",
    "预备金": "Emergency Fund",
    "为什么遇到意外前要留现金？": "Why keep cash before surprises happen?",
    "借款": "Borrowing",
    "什么时候可能有帮助？风险是什么？": "When can it help, and what are the risks?",
    "分散": "Diversification",
    "为什么不要把所有钱放在同一个选择？": "Why avoid putting all your money into one choice?",
    "收入类型": "Income Types",
    "主动收入和被动收入有什么不同？": "What is the difference between active and passive income?",
    "新手提示更多，标准平衡，进阶波动略高": "Beginner has more hints, Standard is balanced, and Advanced has a little more volatility.",
    "教学": "Tutorial",
    "可以在设置中重播或重置教学，不会清除游戏存档": "You can replay or reset the tutorial in settings without clearing the game save.",
    "打开词典": "Open Glossary",
    "重播教学": "Replay Tutorial",
    "保留 ¥10,000 现金": "Keep ¥10,000 Cash",
    "让现金保持在 ¥10,000 以上。": "Keep cash above ¥10,000.",
    "完成一次股票交易": "Complete One Stock Trade",
    "买入或卖出一支虚构股票。": "Buy or sell one fictional stock.",
    "查看房产机会": "View a Property Deal",
    "看过一张房产机会卡。": "Read one property deal card.",
    "偿还一部分贷款": "Repay Part of a Loan",
    "偿还至少 ¥1,000 债务。": "Repay at least ¥1,000 of debt.",
    "完成一次小生意升级": "Upgrade a Small Business",
    "让一个小生意完成升级。": "Upgrade one small business.",
    "购买一张保险": "Buy One Insurance Policy",
    "为某类风险准备一张保单。": "Prepare one policy for a type of risk.",
    "连续两个月正现金流": "Positive Cash Flow for Two Months",
    "连续 2 个月月现金流为正。": "Keep monthly cash flow positive for 2 months.",
    "查看一次财务面板": "Open the Finance Panel Once",
    "打开财务资讯，理解现金流。": "Open finance info and understand cash flow.",
    "完成一次学习事件": "Complete One Learning Event",
    "完成一次学习格事件。": "Complete one learning-space event.",
    "被动收入提升 ¥500": "Raise Passive Income by ¥500",
    "让被动收入比任务开始时多 ¥500。": "Increase passive income by ¥500 from mission start.",
    "月结": "Payday",
    "房产": "Property",
    "持有": "Holding",
    "市场未变化": "No market change yet",
    "行情": "Ticker",
    "小店": "Shop",
    "经营": "Operate",
    "需求": "Demand",
    ...
    "银行与行情": "Banking & Tickers",
    "商业区": "Business District",
    "商店与办公": "Shops & Offices",
    "创业区": "Startup District",
    "小店实验街": "Shop Lab Street",
    "教育区": "Education District",
    "学校与课程": "School & Courses",
    "医疗区": "Health District",
    "医院与诊所": "Hospital & Clinics",
    "公园休闲区": "Park District",
    "喷泉与小桥": "Fountain & Bridge",
    "公共服务区": "Public Services",
    "邮局与路灯": "Post Office & Lamps",
    "起点": "Start",
    "现金流路": "Cashflow Road",
    "住宅花园": "Garden Homes",
    "公寓": "Apartments",
    ...
    "公园喷泉": "Park Fountain",
    "工资收入": "Salary Income",
    "失业期间工资暂停": "Salary paused during unemployment",
    "主动收入": "Active Income",
    "人生事件收入": "Life Event Income",
    "持续事件造成的收入变化": "Ongoing income changes from life events",
    "小生意主動收入": "Active Business Income",
    "需要你投入時間的淨利": "Net profit that needs your time",
    "小生意被動收入": "Passive Business Income",
    "系統建立後較少時間維持": "Less time needed after systems are built",
    "租金收入": "Rental Income",
    "房地产每月租金": "Monthly property rent",
    "股票股息": "Stock Dividends",
    "Payday 发放时进入现金": "Paid into cash on Payday",
    "其他被动收入": "Other Passive Income",
    "生意或证券流入": "Business or security income",
    "生活支出": "Living Expenses",
    "职业基础支出": "Base career expenses",
    "保险保费": "Insurance Premiums",
    "保險保費": "Insurance Premiums",
    "已购保单每月固定支出": "Monthly cost of active policies",
    "已購保單每月固定支出": "Monthly cost of active policies",
    "人生持续支出": "Ongoing Life Expenses",
    "人生持續支出": "Ongoing Life Expenses",
    "医疗、家庭或工作压力持续影响": "Ongoing health, family, or work pressure",
    "醫療、家庭或工作壓力持續影響": "Ongoing health, family, or work pressure",
    "房贷月供": "Mortgage Payment",
    "房貸月供": "Mortgage Payment",
    "房地产贷款月付款": "Monthly real estate loan payment",
    "房地產貸款月付款": "Monthly real estate loan payment",
    "房产支出": "Property Expenses",
    "房產支出": "Property Expenses",
    "管理费与维修准备金": "Management and repair reserve",
    "管理費與維修準備金": "Management and repair reserve",
    "其他贷款月供": "Other Loan Payments",
    "其他貸款月供": "Other Loan Payments",
    "非房贷负债月付款": "Non-mortgage debt payments",
    "非房貸負債月付款": "Non-mortgage debt payments",
    "贷款总额": "Total Loans",
    "貸款總額": "Total Loans",
    "银行、个人贷款与房贷余额": "Bank, personal loan, and mortgage balances",
    "銀行、個人貸款與房貸餘額": "Bank, personal loan, and mortgage balances",
    "信用分": "Credit Score",
    "普通": "Standard",
    "影响额度和利率": "Affects limits and interest rates",
    "当前利率": "Current Interest Rate",
    "當前利率": "Current Interest Rate",
    "个人月利率": "Personal monthly rate",
    "個人月利率": "Personal monthly rate",
    "税务预估": "Estimated Taxes",
    "稅務預估": "Estimated Taxes",
    "游戏简化税务": "Simplified game tax",
    "遊戲簡化稅務": "Simplified game tax",
    "税率": "Tax rate",
    "稅率": "Tax rate",
    "景气状态": "Economic Climate",
    "稳定": "Stable",
    "市场和工作机会都比较平稳。": "Markets and job opportunities are relatively steady.",
    "净月现金流": "Net Monthly Cash Flow",
    "月结日实际增加的现金": "Cash actually added on Payday",
    "手上现金": "Cash on Hand",
    "可以投资或应急": "Available for investing or emergencies",
    "现金 + 资产 - 负债": "Cash + assets - liabilities",
    "暂无其他资产": "No other assets yet",
    "本轮重点资产在房地产中心。": "Main assets are shown in the real estate center.",
    "暂无负债": "No liabilities yet",
    "借款或买入房产后会增加月供。": "Borrowing or buying property may add monthly payments.",
    "余额": "balance",
    "月供": "monthly payment",
    "信用额度": "Credit Limit",
    "每月月供": "Monthly Payment",
    "每月总月供": "Total Monthly Payments",
    "每月總月供": "Total Monthly Payments",
    "利率": "Interest Rate",
    "房贷余额": "Mortgage Balance",
    "房貸餘額": "Mortgage Balance",
    "打开银行中心": "Open Bank Center",
    "打開銀行中心": "Open Bank Center",
    "个人贷款": "Personal Loan",
    "個人貸款": "Personal Loan",
    "申请个人贷款": "Apply for a Personal Loan",
    "申請個人貸款": "Apply for a Personal Loan",
    "再融资": "Refinance",
    "再融資": "Refinance",
    "查看再融资": "View Refinancing",
    "查看再融資": "View Refinancing",
    "破产保护": "Bankruptcy Protection",
    "破產保護": "Bankruptcy Protection",
    "最近银行记录": "Recent Bank Records",
    "最近銀行記錄": "Recent Bank Records",
    "贷款会马上增加现金，但之后每个月都要还款。信用分数和利率会影响可借额度与月供。": "A loan increases cash right away, but you must repay it each month. Credit score and interest rates affect how much you can borrow and the payment.",
    "貸款會馬上增加現金，但之後每個月都要還款。信用分數和利率會影響可借額度與月供。": "A loan increases cash right away, but you must repay it each month. Credit score and interest rates affect how much you can borrow and the payment.",
    "银行记录": "Bank Records",
    "暂无记录": "No records yet",
    "贷款、利率变化、再融资和破产保护会记录在这里。": "Loans, rate changes, refinancing, and protection records appear here.",
    "每月保费": "Monthly Premiums",
    "已购保单": "Active Policies",
    "失业状态": "Employment Status",
    "目前有工作": "Currently employed",
    "税务余额": "Tax Balance",
    "预备金月数": "Emergency Fund Months",
    "建议预备金": "Suggested Emergency Fund",
    "景气循环": "Economic Cycle",
    "高度警告": "High Alert",
    "保险和预备金不同：保险分担特定风险，预备金是随时可用的现金。": "Insurance and emergency funds are different: insurance shares specific risks, while an emergency fund is ready cash.",
    "游戏中的税务、保险、医疗和法律规则都是简化学习规则，不是真实建议。": "Tax, insurance, medical, and legal rules in this game are simplified learning rules, not real advice.",
    "抽人生事件": "Draw Life Event",
    "寻找工作": "Find Work",
    "税务摘要": "Tax Summary",
    "人生事件记录": "Life Event Records",
    "全部": "All",
    "家庭": "Family",
    "法律": "Legal",
    "奖励": "Reward",
    "医疗、工作、家庭、税务、保险和法律事件会记录在这里。": "Health, work, family, tax, insurance, and legal events appear here.",
    "持有数量": "Holdings",
    "总市值": "Total Market Value",
    "房贷总额": "Total Mortgage",
    "房产净值": "Property Equity",
    "每月租金": "Monthly Rent",
    "每月支出": "Monthly Expenses",
    "每月现金流": "Monthly Cash Flow",
    "还没有房地产": "No real estate yet",
    "遇到房地产机会时，可以从首付、房贷和租金开始判断。": "When a real estate deal appears, start with down payment, mortgage, and rent.",
    "房地产交易记录": "Real Estate Records",
    "购买、出售、维修和市场变化会记录在这里。": "Purchases, sales, repairs, and market changes appear here.",
    "股票总市值": "Stock Market Value",
    "总投入成本": "Total Cost",
    "未实现损益": "Unrealized Gain / Loss",
    "已实现损益": "Realized Gain / Loss",
    "累计股息": "Total Dividends",
    "持有股票": "Stocks Held",
    "组合风险": "Portfolio Risk",
    "最大单一占比": "Largest Holding",
    "上涨": "Up",
    "下跌": "Down",
    "低": "Low",
    "产业": "Sector",
    "组合占比": "Portfolio Share",
    "市值": "Value",
    "价值": "Value",
    "净值": "Equity",
    "股": "shares",
    "支": "stocks",
    "目前没有明显集中提醒。分散可以减少一次事件带来的影响，但不能消除所有风险。": "No major concentration warning right now. Diversification can reduce the impact of one event, but cannot remove all risk.",
    "还没有股票": "No stocks yet",
    "落到股票机会格后，可以从价格、风险和分散开始判断。": "When you land on a stock deal, start with price, risk, and diversification.",
    "股票交易记录": "Stock Records",
    "买入、卖出、股息和迁移会记录在这里。": "Buys, sells, dividends, and migrations appear here.",
    "持有數量": "Holdings",
    "個": "items",
    "總市值": "Total Market Value",
    "每月營收": "Monthly Revenue",
    "每月成本": "Monthly Costs",
    "每月淨利": "Monthly Net Profit",
    "累計獲利": "Total Profit",
    "平均風險": "Average Risk",
    "被動占比": "Passive Share",
    "等級": "Level",
    ...
    "價值": "Value",
    "被動": "Passive",
    "還沒有小生意": "No small business yet",
    "目前沒有明顯集中提醒。分散可以降低一次事件的影響，但不能消除所有風險。": "No major concentration warning right now. Diversification can reduce one event's impact, but cannot remove all risk.",
    "落到小生意機會格後，可以從營收、成本和淨利開始判斷。": "When you land on a small business deal, start with revenue, costs, and net profit.",
    "小生意交易記錄": "Small Business Records",
    "投資、升級、收入、事件和出售會記錄在這裡。": "Investments, upgrades, income, events, and sales appear here.",
    "市場變化": "Market Change",
    "出售": "Sale",
    ...
    "新的进度": "New Progress",
    "新的進度": "New Progress",
    "新的进度已记录。": "New progress has been recorded.",
    "新的進度已記錄。": "New progress has been recorded.",
    "里程碑": "Milestone",
    "成就": "Achievement",
    "徽章": "Badge",
    "完成第一回合": "Finished the First Round",
    "你已经开始现金流城市的第一步。": "You have taken your first step in Cashflow City.",
    "你已經開始現金流城市的第一步。": "You have taken your first step in Cashflow City.",
    "第一次正现金流": "First Positive Cash Flow",
    "第一次正現金流": "First Positive Cash Flow",
    "每月留下的钱开始变多了。": "You are starting to keep more money each month.",
    "每月留下的錢開始變多了。": "You are starting to keep more money each month.",
    "第一桶金": "First Cash Cushion",
    "保留至少 ¥10,000 现金。": "Keep at least ¥10,000 in cash.",
    "保留至少 ¥10,000 現金。": "Keep at least ¥10,000 in cash.",
    "现金达到 ¥10,000。": "Reach ¥10,000 in cash.",
    "現金達到 ¥10,000。": "Reach ¥10,000 in cash.",
    "负债降温": "Debt Cooling Down",
    "負債降溫": "Debt Cooling Down",
    "总负债低于 ¥10,000。": "Keep total debt under ¥10,000.",
    "總負債低於 ¥10,000。": "Keep total debt under ¥10,000.",
    "房地产机会": "Real Estate Opportunity",
    "房地產機會": "Real Estate Opportunity",
    "地铁旁小套房": "Small Studio Near Transit",
    "地鐵旁小套房": "Small Studio Near Transit",
    "面积不大，但通勤方便，适合刚工作的租客。 小房子也可以是资产，重点是租金扣掉贷款和维修后还剩多少。": "It is small, but the commute is convenient for new workers. A small home can be an asset, but the key is what remains after rent, the loan, and repairs.",
    "面積不大，但通勤方便，適合剛工作的租客。 小房子也可以是資產，重點是租金扣掉貸款和維修後還剩多少。": "It is small, but the commute is convenient for new workers. A small home can be an asset, but the key is what remains after rent, the loan, and repairs.",
    "小房子也可以是资产，重点是租金扣掉贷款和维修后还剩多少。": "A small home can be an asset, but the key is what remains after rent, the loan, and repairs.",
    "小房子也可以是資產，重點是租金扣掉貸款和維修後還剩多少。": "A small home can be an asset, but the key is what remains after rent, the loan, and repairs.",
    "类型": "Type",
    "類型": "Type",
    "小套房": "Small Studio",
    "总价": "Total Price",
    ...
    "贷款金额": "Loan Amount",
    "貸款金額": "Loan Amount",
    "每月房贷": "Monthly Mortgage",
    "每月房貸": "Monthly Mortgage",
    "管理与维修": "Management & Repairs",
    "管理與維修": "Management & Repairs",
    "每月净现金流": "Monthly Net Cash Flow",
    "每月淨現金流": "Monthly Net Cash Flow",
    "预计房产净值": "Estimated Home Equity",
    "預計房產淨值": "Estimated Home Equity",
    "风险等级": "Risk Level",
    "風險等級": "Risk Level",
    "地段等级": "Location Grade",
    "地段等級": "Location Grade",
    "儿童说明": "Kid-Friendly Note",
    "兒童說明": "Kid-Friendly Note",
    "房贷购买": "Buy with Mortgage",
    "房貸購買": "Buy with Mortgage",
    "现金购买": "Buy with Cash",
    "現金購買": "Buy with Cash",
    "节奏": "Pace",
    "節奏": "Pace",
    "提醒": "Reminder",
    "前 10 回合每回合最多一个重点": "At most one main focus in each of the first 10 rounds",
    "前 10 回合每回合最多一個重點": "At most one main focus in each of the first 10 rounds",
    "不会强行生成假交易": "The game will not force fake transactions",
    "不會強行生成假交易": "The game will not force fake transactions",
    "为什么？": "Why?",
    "為什麼？": "Why?",
    "下一步": "Next",
    "例子": "Example",
