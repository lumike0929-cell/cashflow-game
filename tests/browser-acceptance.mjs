...
try {
  ...
  for (const viewport of [
    ...
    const overflow = await page.evaluate(() => ({
      ...
      intersections: document.querySelectorAll(".city-intersection").length,
      progressBeacons: document.querySelectorAll(".progress-beacon").length,
      ...
      avatarEars: document.querySelectorAll("#avatarAnchor .avatar-ear").length,
      avatarSilhouette: Boolean(document.querySelector("#avatarAnchor .avatar-career-silhouette")),
      diceRing: Boolean(document.querySelector(".dice-ring")),
    ...
    assert.ok(overflow.intersections >= 6);
    assert.ok(overflow.progressBeacons >= 4);
    assert.ok(overflow.zoneBadges >= 6);
    assert.ok(overflow.avatarProps >= 1);
    assert.equal(overflow.avatarEars, 2);
    assert.equal(overflow.avatarSilhouette, true);
    assert.equal(overflow.diceRing, true);
