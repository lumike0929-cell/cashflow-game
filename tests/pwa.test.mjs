...
test("PWA manifest, icons, and service worker shell are present", async () => {
  ...
  assert.match(worker, /cashflow-game-shell-playtest-polish-20260801/);
  assert.match(worker, /funSystem\.js/);
  assert.match(worker, /localMultiplayerSystem\.js/);
...
test("public beta release information is complete and localized", () => {
  ...
  assert.equal(releaseInfo.serviceWorkerVersion, "cashflow-game-shell-playtest-polish-20260801");
